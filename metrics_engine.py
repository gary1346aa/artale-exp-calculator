...
accumulated gains, baseline tracking, and level-up ETA calculations in Traditional Chinese.
Handles session initialization, measurement baselines, auto-start, and hotkey state transitions.
"""
...
class MeasurementState(enum.Enum):
  IDLE = "IDLE"          # Measurement not started yet or reset
  RUNNING = "RUNNING"    # Actively measuring hunting time and EXP gains
  PAUSED = "PAUSED"      # Paused (timer suspended, auto-start disabled once)
...
class ExpMetricsEngine:
  """Core metrics engine tracking session initial EXP, measurement baseline, and rates."""

  def __init__(self, window_seconds: int = 3600):
    self.window_seconds = window_seconds

    # Session-wide initial EXP (captured once when app launches, never reset until app restarts)
    self.initial_exp: Optional[int] = None
    self.initial_exp_percent: Optional[float] = None
    self.initial_timestamp: Optional[float] = None

    # Current measurement baseline (starting reference for this measurement run)
  ...
  def set_auto_start(self, enabled: bool) -> None:
    """Toggles or sets the auto-start state."""
  ...
  def toggle_start_stop(self) -> str:
    """F7 hotkey: toggles between start and pause."""
    if self.state == MeasurementState.RUNNING:
  ...
  def reset_measurement(self, disable_auto_start: bool = True) -> None:
    """F8 hotkey: resets current measurement without starting, disabling auto-start once.

    Initial EXP is preserved.
    """
  ...
  def add_sample(self, exp_value: int, exp_percent: Optional[float],
    ...
    # 1. Capture session initial EXP (set once on launch, never cleared on reset)
    ...
    if self.state != MeasurementState.RUNNING:
      ...
      if self.baseline_exp is not None and exp_value > self.baseline_exp:
        is_increasing = True
      elif (self.baseline_exp_percent is not None and exp_percent is not None and
            exp_percent > self.baseline_exp_percent):
        is_increasing = True

      if self.auto_start_enabled and is_increasing:
        # Automatically start or resume measurement
        self.start_measurement()
        # Fall through to compute gain from baseline!
      elif self.state == MeasurementState.IDLE and not self.auto_start_enabled:
        # When idle and auto-start is not armed, baseline floats with current EXP
        self.baseline_exp = exp_value
        self.baseline_exp_percent = exp_percent
        self.baseline_timestamp = now
    ...
    if self.state == MeasurementState.RUNNING:
      if not self.samples:
        ...
        delta_pct = (sample.exp_percent - prev.exp_percent) if (
            sample.exp_percent is not None and prev.exp_percent is not None) else 0.0
        ...
        if (prev.exp_percent is not None and sample.exp_percent is not None and
          ...
          # Decreased without wrap (death penalty / character switch)
          pass
  ...
  def get_metrics(self, now: Optional[float] = None) -> Dict[str, Any]:
    ...
    if self.state == MeasurementState.IDLE or self.measurement_start_time is None:
      ...
      if self.state == MeasurementState.PAUSED and self.pause_start_time is not None:
    ...
    if self.initial_exp is not None:
      init_pct_str = f"{self.initial_exp_percent:.2f}%" if self.initial_exp_percent is not None else "--"
    ...
    if self.baseline_exp is not None:
      base_pct_str = f"{self.baseline_exp_percent:.2f}%" if self.baseline_exp_percent is not None else "--"
    ...
    rate_pct_per_sec = (pct_1m / dt_1m) if dt_1m > 5.0 else (
