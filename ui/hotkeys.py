...
On Windows: Uses Win32 RegisterHotKey (F6~F9).
On macOS: Uses native Carbon RegisterEventHotKey (⌘6~⌘9).
"""
...
class MacOSCarbonHotkeys:
  """Registers system-wide hotkeys on macOS via Carbon framework (⌘6~⌘9)."""
  ...
  def _setup(self):
    ...
    try:
      ...
      # Carbon Modifiers: cmdKey = 0x0100 (256)
      cmd_key = 0x0100

      # Keycodes: 6: 22, 7: 26, 8: 28, 9: 25
      HOTKEY_SPECS = [
          (1006, 22, cmd_key, "⌘6"),
          (1007, 26, cmd_key, "⌘7"),
          (1008, 28, cmd_key, "⌘8"),
      ...
      for action_id, keycode, mods, desc in HOTKEY_SPECS:
        ...
        if res == 0 and ref.value:
          ...
          logger.info("[CARBON] Registered global hotkey: %s (id=%d)", desc, reg_id)
        else:
          logger.warning("[CARBON] RegisterEventHotKey %s returned status %d", desc, res)
        reg_id += 1
    except Exception as e:
      logger.warning("Error initializing Carbon hotkeys on macOS: %s", e)

  def cleanup(self):
    """Unregisters all Carbon hotkeys and removes event handler."""
    if not self.carbon:
      return
...
class HotkeyWorker(QThread):
  """Global hotkey listener for Windows (F6~F9) and macOS (⌘6~⌘9)."""
